...
struct ch101_comms_struct {
	__u16   len;
	__u8    *buf;
};

ch_thresholds_t chirp_ch101_thresholds = { 
	.threshold = {
		{0, 	5000},		/* threshold 0 */
		{26,	2000},		/* threshold 1 */
		{39,	800},		/* threshold 2 */
...
int chbsp_i2c_write(ch_dev_t * dev_ptr, uint8_t * data, uint16_t n)
{
	...
	msg.addr = ch101_data->i2c_addr;
	msg.flags = 0;
	msg.len = n;
	msg.buf = data;
	ret = i2c_transfer(ch101_data->i2c->adapter, &msg, 1);
	if (ret == 1) {
		ret = 0;
	} else {
		dev_err(&ch101_data->i2c->dev, " ch101 w err\n");
		ret = -EIO;
	}
                        ...
                        uint8_t * data_ptr, uint16_t num_bytes)
{
	...
	if (ret == 1)
	{
		...
		dev_err(&ch101_data->i2c->dev, " ch101 w16 err\n");
		ret = -EIO;
	}
...
int chbsp_i2c_read(ch_dev_t * dev_ptr, uint8_t * data, uint16_t len)
{
	...
	if (ret == 1) {
		...
		dev_err(&ch101_data->i2c->dev, " ch101 r err\n");
		ret = -EIO;
	}
                       ...
                       uint8_t * data_ptr, uint16_t num_bytes)
{
	...
	if (ret == 1)
	{
		...
		dev_err(&ch101_data->i2c->dev, " ch101 r16-w err\n");
		ret = -EIO;
	}
	...
	if (ret == 1)
	{
		...
		dev_err(&ch101_data->i2c->dev, " ch101 r16-r err\n");
		ret = -EIO;
	}
...
{
	if (ch101_data->i2c_rst_gpio >= 0) {
		gpio_direction_output(ch101_data->i2c_rst_gpio, 0);
        // gpio_set_value(ch101_data->i2c_rst_gpio, 0);
		if(gpio_get_value(ch101_data->i2c_rst_gpio) != 0)
		{
			dev_err(&ch101_data->i2c->dev, "ch101 rst low err.");
		} else {
			dev_err(&ch101_data->i2c->dev, "ch101 rst low success.");
		}
	}
}
...
{
	if (ch101_data->i2c_rst_gpio >= 0) {
		gpio_direction_output(ch101_data->i2c_rst_gpio, 1);
        // gpio_set_value(ch101_data->i2c_rst_gpio, 1);
		if(gpio_get_value(ch101_data->i2c_rst_gpio) != 1)
		{
			dev_err(&ch101_data->i2c->dev, "ch101 reset high err.");
		} else {
			dev_err(&ch101_data->i2c->dev, "ch101 reset high success.");
		}
	}
}
...
{
	if (ch101_data->prog_gpio >= 0) {
        gpio_set_value(ch101_data->prog_gpio, 1);
		if(gpio_get_value(ch101_data->prog_gpio) != 1)
		{
			dev_err(&ch101_data->i2c->dev, "ch101 prog high err.");
		} else {
			//dev_err(&ch101_data->i2c->dev, "ch101 prog high success.");
		}
	}
}
...
{
	if (ch101_data->prog_gpio >= 0) {
        gpio_set_value(ch101_data->prog_gpio, 0);
		if(gpio_get_value(ch101_data->prog_gpio) != 0)
		{
			dev_err(&ch101_data->i2c->dev, "ch101 prog low err.");
		} else {
			//dev_err(&ch101_data->i2c->dev, "ch101 prog low success.");
		}
	}
}
...
void chbsp_group_set_io_dir_out(ch_group_t * grp_ptr)
{
	...
	if (ch101_data->irq_gpio >= 0) {
		ret = gpio_direction_output(ch101_data->irq_gpio, 0);
		if (ret < 0) {
			dev_err(&ch101_data->i2c->dev,
				"%s gpio_dir_inp(%d) ERR:%d\n",
				__func__, ch101_data->irq_gpio, ret);
		}
	}
}
...
void chbsp_group_set_io_dir_in(ch_group_t * grp_ptr)
{
	...
	if (ch101_data->irq_gpio >= 0) {
		ret = gpio_direction_input(ch101_data->irq_gpio);
		if (ret < 0) {
			dev_err(&ch101_data->i2c->dev,
				"%s gpio_dir_inp(%d) ERR:%d\n",
				__func__, ch101_data->irq_gpio, ret);
...
void chbsp_group_pin_init(ch_group_t * grp_ptr)
{
	...
	dev_err(&ch101_data->i2c->dev, "ch101 enter chbsp_group_pin_init");

    /* default device specific parameters */
	ch101_data->irq_gpio = -1;
    ch101_data->prog_gpio = -1;
    ch101_data->i2c_rst_gpio = -1;
	/* device tree parameters */
	ch101_data->irq_gpio = of_get_named_gpio(ch101_data->i2c->dev.of_node, "irq_gpios", 0);
	if (ch101_data->irq_gpio < 0) {
		dev_err(&ch101_data->i2c->dev,
				"%s gpio irq get named(%d) \n",
				__func__, ch101_data->irq_gpio);
	}
    ch101_data->prog_gpio = of_get_named_gpio(ch101_data->i2c->dev.of_node, "prog_gpios", 0);
	if (ch101_data->prog_gpio < 0) {
		dev_err(&ch101_data->i2c->dev,
				"%s gpio prog get named(%d) \n",
				__func__, ch101_data->prog_gpio);
	}
    ch101_data->i2c_rst_gpio = of_get_named_gpio(ch101_data->i2c->dev.of_node, "i2c_rst_gpios", 0);
	if (ch101_data->i2c_rst_gpio < 0) {
		dev_err(&ch101_data->i2c->dev,
				"%s gpio rst get named(%d) \n",
				__func__, ch101_data->i2c_rst_gpio);
	}

    if (ch101_data->irq_gpio >= 0) {
		ret = gpio_request(ch101_data->irq_gpio, "irq_gpio");
		if (ret) {
			dev_err(&ch101_data->i2c->dev,
		...
		tdk_ultra_irq_num = gpio_to_irq(ch101_data->irq_gpio);
		if (tdk_ultra_irq_num <= 0) {
			dev_err(&ch101_data->i2c->dev, 
				"%s gpio_to_irq(%d) ERR:%d\n",
				__func__, ch101_data->irq_gpio, tdk_ultra_irq_num);
		}
		...
		// 				"st_tof_sensor",
		// 				NULL);
		ret = request_irq(tdk_ultra_irq_num,
						tdk_ultra_intr_handler,
						IRQF_TRIGGER_RISING|IRQF_ONESHOT,
						"st_tof_sensor",
						CHIRP_NAME);
		...
		if (ret) {
			pr_err("ch101: Failed to Register IRQ handler,"
					" GPIO = %d, st_tof_irq_num = %d\n",
					ch101_data->irq_gpio, tdk_ultra_irq_num);
		} else {
			printk("ch101: Registered IRQ handler, GPIO = %d, "
					"st_tof_irq_num = %d\n",
					ch101_data->irq_gpio, tdk_ultra_irq_num);
			irq_handler_registered = 1;
		}

		disable_irq(tdk_ultra_irq_num);
    }

    if (ch101_data->prog_gpio >= 0) {
		ret = gpio_request(ch101_data->prog_gpio, "prog_gpio");
		if (ret) {
			dev_err(&ch101_data->i2c->dev,
				"%s gpio_request(%d %s) ERR:%d\n",
				__func__, ch101_data->prog_gpio, CHIRP_NAME, ret);
		}

		ret = gpio_direction_output(ch101_data->prog_gpio, 0);
		if (ret < 0) {
			dev_err(&ch101_data->i2c->dev,
				"%s gpio_prog(%d) ERR:%d\n",
				__func__, ch101_data->prog_gpio, ret);
		}
    }

    if (ch101_data->i2c_rst_gpio >= 0) {
		ret = gpio_request(ch101_data->i2c_rst_gpio, "i2c_rst_gpio");
		if (ret) {
			dev_err(&ch101_data->i2c->dev,
				"%s gpio_request(%d %s) ERR:%d\n",
				__func__, ch101_data->i2c_rst_gpio, CHIRP_NAME, ret);
		}

		// ret = gpio_direction_output(ch101_data->i2c_rst_gpio, 0);
		// if (ret < 0) {
		// 	dev_err(&ch101_data->i2c->dev,
		// 		"%s gpio_dir_inp(%d) ERR:%d\n",
		// 		__func__, ch101_data->i2c_rst_gpio, ret);
		// }
    }
...
void chbsp_group_io_clear(ch_group_t * grp_ptr)
{
	...
	if (ch101_data->irq_gpio >= 0) {
        gpio_set_value(ch101_data->irq_gpio, 0);
		if(gpio_get_value(ch101_data->irq_gpio) != 0)
		{
			dev_err(&ch101_data->i2c->dev, "ch101 irq low err.");
		} else {
			//dev_err(&ch101_data->i2c->dev, "ch101 irq low success.");
		}
	}
}
...
{
    if (ch101_data->irq_gpio >= 0) {
        gpio_set_value(ch101_data->irq_gpio, 1);
		if(gpio_get_value(ch101_data->irq_gpio) != 1)
		{
			dev_err(&ch101_data->i2c->dev, "ch101 irq high err.");
		} else {
			//dev_err(&ch101_data->i2c->dev, "ch101 irq high success.");
		}
	}
}
...
void chbsp_board_init(ch_group_t * grp_ptr)
{
  ...
  ch101_data->chirp_group = grp_ptr;
 ...
 *
 * For CH101 sensors only, the multiple detection threshold values are also 
 * displayed.
 */
	...
	if (!chirp_error) {
		...
		if (ch_get_part_number(dev_ptr) == CH201_PART_NUMBER) {
			...
			if (!chirp_error) {
				...
				printk("ch101     %d\tstart: %2d\tlevel: %d\r\n", i,
						read_thresholds.threshold[i].start_sample,
						read_thresholds.threshold[i].level);
				}
			} else {
				printk("ch101 Device %d: Error during ch_get_thresholds()\r\n", dev_num);
			}
		}
		printk("\n");

	} else {
		printk("ch101 Device %d: Error during ch_get_config()\r\n", dev_num);
	}
...
void periodic_timer_callback(struct timer_list *unused) {

	if (num_triggered_devices > 0) {
		ch_group_trigger(&chirp_group);
	}
	mod_timer(&my_timer, jiffies + msecs_to_jiffies(MEASUREMENT_INTERVAL_MS));
}
...
static int ch101_open(struct inode *inode, struct file *file)
{
	pr_debug("ch101 : %s(%d)\n", __func__, __LINE__);
	return 0;
}

static int ch101_release(struct inode *inode, struct file *file)
{
	pr_debug("ch101 : %s(%d)\n", __func__, __LINE__);
	return 0;
}

static long ch101_ioctl(struct file *file,
		unsigned int cmd, unsigned long arg)
{
	...
	struct ch101_comms_struct comms_struct = {0};
	...
	switch (cmd) {
		case SEN_ULTRASONIC_IOCTL_MEASURE:
			...
			// printk("ch101 Range: %d mm  Amplitude: %u  \r\n", range, amplitude);

			// copy to user buffer the read transfer
			ret = copy_to_user(data_ptr, (uint8_t *)&range, 4);

			if (ret) {
				pr_err("ch101: Error at %s(%d) when copy range to user.\n", __func__, __LINE__);
				return -EINVAL;
			}
			...
			break;
		case SEN_ULTRASONIC_IOCTL_MEASURE_STOP:
			measure_status = 0;
			break;
		case SEN_ULTRASONIC_IOCTL_DEVID:
			...
			if (ret) {
				pr_err("ch101: Error at %s(%d) when copy dev_id to user.\n", __func__, __LINE__);
				return -EINVAL;
			}
			break;
...
static const struct file_operations ch101_fops = {
	.owner 			= THIS_MODULE,
	.unlocked_ioctl		= ch101_ioctl,
	.open 			= ch101_open,
	.release 		= ch101_release,
};
...
static int chirp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ch101_data = devm_kzalloc(&client->dev, sizeof(*ch101_data), GFP_KERNEL);
	if (ch101_data == NULL)
		return -ENOMEM;

	ch101_data->i2c = client;
	ch101_data->i2c_addr = CHIRP_I2C_ADDRS;
	i2c_set_clientdata(client, ch101_data);

	grp_ptr = &chirp_group;
	ch101_data->chirp_group = grp_ptr;
	ch101_data->ch101 = dev_ptr;
	chbsp_board_init(ch101_data->chirp_group);

	num_ports = ch_get_num_ports(ch101_data->chirp_group);

    for (dev_num = 0; dev_num < num_ports; dev_num++) {
        dev_ptr = &(chirp_devices[dev_num]);      // init struct in array
        ret |= ch_init(dev_ptr, ch101_data->chirp_group, dev_num,
                            CHIRP_SENSOR_FW_INIT_FUNC);
    }

	ret = !ch101_data->chirp_group;
...
RESET_AND_LOAD:
	 ...
	 */
	printk ("ch101 Configuring sensor(s)...\n");
	for (dev_num = 0; dev_num < num_ports; dev_num++) {
		...
		if (ch_sensor_is_connected(dev_ptr)) {
			...
			// dev_config.thresh_ptr = &chirp_ch101_thresholds;
			...
			// ret = ch_set_thresholds(dev_ptr,&chirp_ch101_thresholds);

			/* Read back and display config settings */
			if (!ret) {
				display_config_info(dev_ptr);
			} else {
				printk("ch101 Device %d: Error during ch_set_config()\n", dev_num);
			}
	...
	if (num_triggered_devices > 0) {
		printk("Initializing sample timer for %dms interval... ", 
	...
	sen_ulsonic_miscdev.fops = &ch101_fops;
...
static int __init ch101_module_init(void)
{
	int ret = 0;

	printk("ch101: module init\n");
	...
	if (ret) {
		...
		printk("ch101: could not add i2c driver\n");
		return ret;
	}
...
static void __exit ch101_module_exit(void)
{

	pr_debug("ch101 : module exit\n");
...
module_init(ch101_module_init);
module_exit(ch101_module_exit);

MODULE_AUTHOR("DDT");
MODULE_DESCRIPTION("CH101 I2C driver");
MODULE_LICENSE("GPL");